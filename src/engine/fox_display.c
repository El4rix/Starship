...
void Display_JetpackThrusters(Player* player) {
    ...
    sp2C = player->unk_16C;     // First thruster
    if (sp2C > 0.2f) {
        sp2C *= 1.1f;
        ...
        Matrix_Push(&gGfxMatrix);
        if ((gLevelMode == LEVELMODE_ALL_RANGE) && (player->state != PLAYERSTATE_START_360)) {
            Matrix_Translate(gGfxMatrix, player->pos.x + 15.0f * COS_DEG(-player->yRot_114 - player->rot.y + 90.0f - 30.0f), 
                                        player->pos.y - 3.0f, 
                                        player->pos.z + 15.0f * SIN_DEG(-player->yRot_114 - player->rot.y + 90.0f - 30.0f), MTXF_APPLY);
        } else if ((player->state == PLAYERSTATE_LEVEL_COMPLETE) && ((gCurrentLevel == LEVEL_CORNERIA) || (gCurrentLevel == LEVEL_VENOM_ANDROSS))) {
            Matrix_Translate(gGfxMatrix, player->pos.x + 10.0f, player->pos.y - 3.0f, player->trueZpos + player->zPath - 10, MTXF_APPLY);
        } else {
            Matrix_Translate(gGfxMatrix, player->pos.x + 10.0f, player->pos.y - 3.0f, player->trueZpos + player->zPath + 10, MTXF_APPLY);
        }
        Matrix_RotateY(gGfxMatrix, -gPlayer[gPlayerNum].camYaw, MTXF_APPLY);
    ...
    sp2C = player->unk_170;     // Repeat for 2nd thruster
    if (sp2C > 0.2f) {
        sp2C *= 1.1f;
        ...
        if ((gLevelMode == LEVELMODE_ALL_RANGE) && (player->state != PLAYERSTATE_START_360)) {
            ...
            Matrix_Translate(gGfxMatrix, player->pos.x - 10.0f, player->pos.y - 3.0f, player->trueZpos + player->zPath + 10.0f, MTXF_APPLY);
        }
        Matrix_RotateY(gGfxMatrix, -gPlayer[gPlayerNum].camYaw, MTXF_APPLY);
        Matrix_Scale(gGfxMatrix, sp2C, sp2C, sp2C, MTXF_APPLY);
...
void Display_BarrelRollShield(Player* player) {
    ...
    if (player->barrelRollAlpha != 0) {
        ...
        if ((player->form == FORM_ON_FOOT) && (gLevelMode == LEVELMODE_ON_RAILS)) {
            Matrix_Translate(gGfxMatrix, 0, 22.0f, 0, MTXF_APPLY);
        }
        Matrix_RotateY(gGfxMatrix, -gPlayer[gPlayerNum].camYaw, MTXF_APPLY);
...
void Display_Player_Update(Player* player, s32 reflectY) {
    ...
    if (player->draw) {
        ...
        if ((player->form == FORM_ON_FOOT) && (gCurrentLevel == LEVEL_SECTOR_Y) && (gLevelMode == LEVELMODE_ALL_RANGE)) { // SY Ship
            Matrix_Push(&gGfxMatrix);
            RCP_SetupDL_30(gFogRed, gFogGreen, gFogBlue, gFogAlpha, gFogNear, gFogFar);
        ...
        if ((player->form == FORM_LANDMASTER) && (!gTurretModeEnabled)) {
            ...
            if (!gTurretModeEnabled) {
                ...
                Matrix_RotateY(gGfxMatrix, (180.0f) * M_DTOR, MTXF_APPLY);
            }
            Matrix_Translate(gGfxMatrix, player->xShake, player->yBob, 0.0f, MTXF_APPLY);
            Matrix_SetGfxMtx(&gMasterDisp);
        }
        ...
        if (gLevelMode == LEVELMODE_ALL_RANGE) {
            if (player->form == FORM_ON_FOOT) {
                if (player->grounded == true) {
                    ...
                    Matrix_Translate(gGfxMatrix, player->groundPos.x, player->yPath, player->groundPos.z,
                                 MTXF_APPLY);
                }
            } else {
