        ...
        if (player->form == FORM_ON_FOOT) {
            if ((player->grounded == true) && (player->baseSpeed > 5)) {
                if ((gGameFrameCount % 8) == 0) {
                    Effect_Effect372_Spawn1(player->pos.x + 5, gGroundHeight - 10, player->trueZpos - 40, 0.1f, 1.0f,
...
void Play_Setup360_SY(void) {
    ...
    if (gPlayer[0].form == FORM_ON_FOOT) {
        boss[1].obj.pos.x = 2500.0f;
        boss[1].obj.pos.z = -7500.0f;
    }
    ...
    if (gPlayer[0].form == FORM_ON_FOOT) {
        boss[2].obj.pos.x = 2500.0f;
        boss[2].obj.pos.z = -7500.0f;
    }
...
void Player_FootCollisionCheck(Player* player) {        // On-Foot Collision
    ...
    if (gLevelMode == LEVELMODE_ALL_RANGE) {                                                    // All-Range 360 collision
        ...
        for (i = 0, scenery360 = gScenery360; i < 200; i++, scenery360++) {                     // 360 Scenery collision
            if (scenery360->obj.status == OBJ_ACTIVE) {
                if (scenery360->obj.id == OBJ_SCENERY_AND_PASSAGE) {
                    if (sqrtf(SQ(player->pos.x - scenery360->obj.pos.x) + SQ(player->pos.z - scenery360->obj.pos.z)) < 1100.0f) {
                        if (player->pos.y < 70.0f) {
                            player->pos.y = 70.0f;
                            player->grounded = true;
                            player->gravity = 0.0f;
                            player->vel.y = 0.0f;
    ...
    if (player->mercyTimer == 0) {         // Collision
                        ...
                        if ((scenery360->obj.id == OBJ_SCENERY_AQ_CORAL_REEF_1) ||
                            ...
                            if (temp_v0 != 0) {
                                ...
                                if ((scenery360->obj.id == OBJ_SCENERY_AND_PATH_WALLS) ||
                                    ...
                                    if (player->yRot_114 > 180.0f) {
                                        player->yRot_114 -= 360.0f;
                                    }
                                    if (player->yRot_114 < -180.0f) {
                                        player->yRot_114 += 360.0f;
                                    }
                                    if (scenery360->obj.rot.y > 180.0f) {
                                        scenery360->obj.rot.y -= 360.0f;
                                    }
                                    if (scenery360->obj.rot.y < -180.0f) {
                                        scenery360->obj.rot.y += 360.0f;
                                    }

                                    if ((player->yRot_114 > scenery360->obj.rot.y + 90.0f) || (player->yRot_114 < scenery360->obj.rot.y - 90.0f)) {
                                        player->yRot_114 = scenery360->obj.rot.y;
                                    } else {
                                        player->yRot_114 = scenery360->obj.rot.y + 180.0f;
                                    }
            ...
            for (i = 0, scenery = gScenery; i < ARRAY_COUNT(gScenery); i++, scenery++) {                  // On-Rails scenery collision
                ...
                if ((scenery->obj.status == OBJ_ACTIVE) 
                    && (Player_CheckHitboxCollision(
                            player, scenery->info.hitbox, &sp98, scenery->obj.pos.x, scenery->obj.pos.y + 25.0f - (25.0f * (player->vel.y / 50.0f)),
                            scenery->obj.pos.z, scenery->obj.rot.x, padB0, scenery->obj.rot.z, 0.0f, 0.0f, 0.0f) > 0)
                    && (Player_CheckHitboxCollision(
                            player, scenery->info.hitbox, &sp98, scenery->obj.pos.x, scenery->obj.pos.y,
                            scenery->obj.pos.z, scenery->obj.rot.x, padB0, scenery->obj.rot.z, 0.0f, 0.0f, 0.0f) <= 0)) {
                    if (gCurrentLevel == LEVEL_ZONESS) {
                            ...
                            if (temp_v0 < 0) {
                                    ...
                                    (scenery->obj.id == OBJ_SCENERY_CO_HIGHWAY_9)) 
                                {
                                    ...
                                    if (scenery->obj.id == OBJ_SCENERY_CO_HIGHWAY_9) {
                                        player->pos.y = scenery->obj.pos.y + 65 - ((player->trueZpos - scenery->obj.pos.z) / 40);
                                    }
        ...
        for (i = 0, boss = &gBosses[0]; i < ARRAY_COUNT(gBosses); i++, boss++) {                        // All Boss Collision
            if ((boss->obj.status == OBJ_ACTIVE) && (boss->obj.id != OBJ_BOSS_KA_FLBASE)) {
        ...
        for (i = 0, actor = &gActors[0]; i < ARRAY_COUNT(gActors); i++, actor++) {                      // All actor collision
            if ((actor->obj.status == OBJ_ACTIVE)/*  && (actor->timer_0C2 == 0) */) {

                if ((gCurrentLevel == LEVEL_AQUAS) && (actor->timer_0C2 != 0) ) {
                    break;
                }

                if (((OBJ_ACTOR_MA_LOCOMOTIVE <= actor->obj.id) && (actor->obj.id <= OBJ_ACTOR_MA_TRAIN_CAR_7))
                    && (Player_CheckHitboxCollision(
                            player, actor->info.hitbox, &sp98, actor->fwork[25] + actor->obj.pos.x,
                            actor->fwork[8] + actor->obj.pos.y + 50.0f - (25.0f * (player->vel.y / 50.0f)), actor->obj.pos.z, actor->fwork[29],
                ...
                } else if (((actor->eventType == EVID_ZERAM_CLASS_CRUISER)
                    || (actor->eventType == EVID_A6_HARLOCK_FRIGATE)
                    || (actor->eventType == EVID_A6_UMBRA_STATION)
                    || (actor->eventType == EVID_SY_SHIP_1)
                    || (actor->eventType == EVID_SY_SHIP_2)
                    || (actor->eventType == EVID_SY_SHIP_3)
                    || (actor->eventType == EVID_SY_SHIP_DESTROYED)
                    || (actor->eventType == EVID_SY_SHIP_4)
                    ...
                    && (Player_CheckHitboxCollision(
                                ...
                                actor->obj.rot.x, actor->obj.rot.y, actor->obj.rot.z, actor->vwork[29].x,
                                actor->vwork[29].y, actor->vwork[29].z + actor->rot_0F4.z) > 0)
                    && (Player_CheckHitboxCollision(
                                player, actor->info.hitbox, &sp98, actor->obj.pos.x, actor->obj.pos.y, actor->obj.pos.z,
                                actor->obj.rot.x, actor->obj.rot.y, actor->obj.rot.z, actor->vwork[29].x,
                                actor->vwork[29].y, actor->vwork[29].z + actor->rot_0F4.z) <= 0)) {
...
void Player_FloorCheck(Player* player) {
    ...
    if (player->form == FORM_ON_FOOT) {
        ...
        player->groundPos.y = player->yPath;    // resets shadow height
    }
            ...
            for (sp144 = 0, scenery360 = gScenery360; sp144 < 200; sp144++, scenery360++) {
                if ((scenery360->obj.status == OBJ_ACTIVE) &&
                    ...
                    if ((scenery360->obj.id == OBJ_SCENERY_FO_MOUNTAIN_2) ||
                        ...
                        if (func_col2_800A3690(&spD4, &spC8, colId, &spBC)) {
                            if (player->form != FORM_ON_FOOT) {     // prevents tower scenery from using mountain collision for player shadow
...
void Player_SetupOnFootShot(Player* player, PlayerShot* shot, PlayerShotId shotId) {
    ...
    if ((player->pos.y > 10.0f) && (player->grounded) && (gCurrentLevel == LEVEL_MACBETH)) {
        Matrix_Translate(gCalcMatrix, 0.0f, 15.0f, 0.0f, MTXF_APPLY);
    } 
    Matrix_RotateX(gCalcMatrix, player->unk_154 * M_DTOR, MTXF_APPLY);
    sp5C.x = 0.0f;
    sp5C.y = 0.0f;
...
void Player_UpdatePath(Player* player) {
    ...
    if ((player->pathChangeTimer != 0) && !gTurretModeEnabled && player->form != FORM_ON_FOOT) {
    ...
    if ((player->form == FORM_ON_FOOT) && (gLevelType == LEVELTYPE_SPACE) && (gCurrentLevel != LEVEL_METEO)) {
        if ((gInputPress->button & R_TRIG) && (player->pos.y <= player->yPath + 50.0f)) {
            Player_PlaySfx(player->sfxSource, NA_SE_ARWING_BRAKE, player->num);
        }
        if ((gInputHold->button & R_TRIG) && (player->pos.y <= player->yPath + 50.0f)) {
            Math_SmoothStepToF(&player->yPathTarget, player->pathChangePitch - 1000.0f, 0.1f, player->pathStep, 0.1f);
            player->baseSpeed = 30.0f;
        } else {
            Math_SmoothStepToF(&player->yPathTarget, player->pathChangePitch, 0.1f, player->pathStep, 0.1f);
        }
...
void Player_OnFootUpdateSpeed(Player* player) {
    ...
    if (gVersusMode) {
        ...
        if (gRunning) {
            if (gCurrentLevel == LEVEL_AQUAS) {
                ...
                || (gCurrentLevel == LEVEL_AREA_6)
                || ((gCurrentLevel == LEVEL_METEO) && (gPathProgress < 20000))) {
                sp2C = 40.0f;
            } else if ((gCurrentLevel == LEVEL_ZONESS) || ((gCurrentLevel == LEVEL_CORNERIA) && (gGroundSurface == SURFACE_WATER))) {
                sp2C = 35.0f;
            } else {
                sp2C = 30.0f;
            }
    ...
    // Tick sprint timer; expiry triggers cooldown (mirrors boostMeter hitting 90 → boostCooldown)
    if (gSuperSprint) {
        if (gSprintTimer > 0) {
            gSprintTimer--;
        }
...
void Player_MoveOnFoot360(Player* player) {
    Vec3f sp78[30];
    f32 sp74;
    f32 sp70;
    f32 var_fv0;
    ...
    if (gCurrentLevel == LEVEL_KATINA) {
        if (fabsf(player->pos.z) > fabs(player->pos.x)) {
            if ((player->pos.z < 530) && (player->pos.z > 0)) {
                player->yPath = 790 - player->pos.z * 1.5;
            }
            if ((player->pos.z > -530) && (player->pos.z < 0)) {
                player->yPath = 790 + player->pos.z * 1.5;
            }
        } else {
            if ((player->pos.x < 530) && (player->pos.x > 0)) {
                player->yPath = 790 - player->pos.x * 1.5;
            }
            if ((player->pos.x > -530) && (player->pos.x < 0)) {
                player->yPath = 790 + player->pos.x * 1.5;
            }
        }
        if (((player->pos.x > -530) && (player->pos.x < 530) && (player->pos.z > -530) && (player->pos.z < 530))) {
            player->yPath += player->baseSpeed;
        }
        if (((fabsf(player->pos.z) > 530) && (fabsf(player->pos.z) < 3000)) || ((fabsf(player->pos.x) > 530) && (fabsf(player->pos.x) < 3000))) {
            if ((player->pos.z > -190) && (player->pos.z < 190)) {
                player->yPath = 10;
            }
            if ((player->pos.x < 190) && (player->pos.x > -190)) {
                player->yPath = 10;
            }
        }
        ...
        player->groundPos.y = 35.0f; // 60
        player->hideShadow = false;

        if (((player->baseSpeed > 0.0f)) && (player->pos.y < 70.0f) && (gBosses[0].health > 0.0f)) {
            if ((gGameFrameCount % 8) == 0) {
                Effect_Effect367_Spawn(player->pos.x, player->groundPos.y, player->pos.z, 1, 7, 0);
            }
        }
    }
    ...
    if (sp74 == 0) {
        //Math_SmoothStepToF(&player->unk_154, 0, 0.075f, 1.0f, 0.00001f);              // delay resetting vertical aim
        //Math_SmoothStepToF(&player->unk_158, 0, 0.15f, 5.0f / 3, 0.00001f);
        //if (!(gInputHold->button & A_BUTTON)/*  && (player->shotTimer < 1) */) {
            Math_SmoothStepToF(&player->unk_154, 0, 0.075f, 1.0f, 0.00001f);
            Math_SmoothStepToF(&player->unk_158, 0, 0.15f, 5.0f / 3, 0.00001f);
        //}
    } else {
        player->shotTimer = 20;
    ...
    // Roll Left/Right
    if ((gInputHold->button & Z_TRIG) && (gInputPress->stick_x > 50) && (player->rollTimer == 0)) {
        AUDIO_PLAY_SFX(NA_SE_PASS, gDefaultSfxSource, 0);
        player->knockback.x = 25.0f * cos(player->camYaw);
        player->knockback.z = 25.0f * sin(player->camYaw);
        player->rot.z = 370;
    ...
    if ((player->grounded) && !(gInputHold->button & Z_TRIG)) {
        player->rollTimer = 0;
    }
    if (player->barrelRollAlpha > 0.0f) {
        player->barrelRollAlpha -= 30.0f;
    }
    ...
    // Limit Height
    if (player->pos.y > (player->pathHeight + gJumpPos)) {
        player->pos.y = (player->pathHeight + gJumpPos);
        player->vel.y = 0.1f;
        player->knockback.y = 0.0f;
    }
    ...
    if (player->grounded) {
        gJumpPos = player->pos.y;

        if (((player->baseSpeed > 1.0f) && (gCurrentLevel != LEVEL_SECTOR_Y))
        || ((gCurrentLevel == LEVEL_SECTOR_Y) && ((player->vel.z > 5) || (player->vel.z < -5)) && (player->pos.z > -1000) && (player->pos.z < 1000))) {
    ...
    if ((gInputPress->button & Z_TRIG) && player->grounded) {
        player->vel.y = 20.0f;
        if ((gCurrentLevel == LEVEL_FORTUNA) && (player->pos.y > 500.0f)) {
            player->pos.y += 30.0f;
        }
    }
    ...
    if (player->pos.y < player->groundPos.y) {      // don't fall below shadow
        player->grounded = true;
        player->pos.y = player->groundPos.y;
        player->vel.y = 0.0f;
    } 
    if (player->pos.y < player->yPath) {     // don't fall below yPath
    ...
    player->yPath = 0;
    if (player->mercyTimer != 0) {      // don't fall through collision just because someone shot you
        player->yPath = player->pos.y;
    }
...
void Player_MoveOnFootRails(Player* player) {
    ...
    if (gCurrentLevel == LEVEL_TITANIA) {
        ...
        if ((gPathProgress > 22000.0f + 0.0f) && (gPathProgress < 22000.0f + 42.0f)) {
            ...
            Actor_Initialize(rock1);
            rock1->obj.status = OBJ_INIT;
            rock1->obj.pos.x = 0.0f;
            rock1->obj.pos.y = -1000.0f;
            rock1->obj.pos.z = -gPathProgress - 10000.0f;
            rock1->obj.rot.z = 180.0f;
            rock1->obj.id = OBJ_ACTOR_ME_MOLAR_ROCK;
            Object_SetInfo(&rock1->info, rock1->obj.id);

            MeMolarRock* rock2 = &gActors[58];
            Actor_Initialize(rock2);
            rock2->obj.status = OBJ_INIT;
            rock2->obj.pos.x = 0.0f;
            rock2->obj.pos.y = -1000.0f;
            rock2->obj.pos.z = -gPathProgress - 12500.0f;
            rock2->obj.rot.z = 180.0f;
            rock2->obj.rot.z = 180.0f;
            rock2->obj.id = OBJ_ACTOR_ME_MOLAR_ROCK;
            Object_SetInfo(&rock2->info, rock2->obj.id);

            MeMolarRock* rock3 = &gActors[57];
            Actor_Initialize(rock3);
            rock3->obj.status = OBJ_INIT;
            rock3->obj.pos.x = 0.0f;
            rock3->obj.pos.y = -1000.0f;
            rock3->obj.pos.z = -gPathProgress - 15000.0f;
            rock3->obj.rot.z = 180.0f;
            rock3->obj.id = OBJ_ACTOR_ME_MOLAR_ROCK;
            Object_SetInfo(&rock3->info, rock3->obj.id);

            MeTunnel* tunnel = &gScenery[49];
            Scenery_Initialize(tunnel);
            tunnel->obj.status = OBJ_INIT;
            tunnel->obj.pos.x = 0.0f;
            tunnel->obj.pos.y = -1250.0f;
            tunnel->obj.pos.z = -gPathProgress - 13550.0f;
            tunnel->obj.rot.y = 180.0f;
            tunnel->obj.id = OBJ_SCENERY_ME_TUNNEL;
            Object_SetInfo(&tunnel->info, tunnel->obj.id);
        }

        if ((gPathProgress > 122000.0f + 0.0f) && (gPathProgress < 122000.0f + 42.0f)) {
            MeMolarRock* rock1 = &gActors[59];
            Actor_Initialize(rock1);
            rock1->obj.status = OBJ_INIT;
            rock1->obj.pos.x = 0.0f;
        ...
        if ((gPathProgress > 289000.0f + 0.0f) && (gPathProgress < 289000.0f + 42.0f)) {
            MeMolarRock* rock1 = &gActors[59];
            Actor_Initialize(rock1);
            rock1->obj.status = OBJ_INIT;
            rock1->obj.pos.x = 0.0f;
    ...
    sp74 = gInputPress->stick_y * 0.65f; // 0.8
    ...
    Math_SmoothStepToF(&player->rot.y, -sp74 * gPlayerTurnStickMod * 0.6f, 0.25f, gPlayerTurnRate, 0.001f);
    Math_SmoothStepToF(&player->rot.z, -sp74 * gPlayerTurnStickMod * 0.2f * player->baseSpeed / 15.0f, 0.1f, 5.0f, 0.001f);
    ...
    if ((player->yRot_114 < -40) || (player->yRot_114 > 40)) {
        player->yRotVel_11C *= fabsf(40 / player->yRot_114);
    }
    ...
    if ((player->pos.y < player->groundPos.y) && ((gLevelType == LEVELTYPE_PLANET) || (gCurrentLevel == LEVEL_METEO))) {      // don't fall below shadow
        player->grounded = true;
        player->pos.y = player->groundPos.y;
        player->vel.y = 0.0f;
    } 
...
void Player_Setup(Player* playerx) {
    ...
    /* AqBacoon* core = &gBosses[0];               // Boss test
    ...
    /* AqBacoon* core = &gScenery[0];               // Scenery Rails test
    ...
    /* Scenery* core = &gScenery360[35];               // Scenery 360 test
...
void Player_Update(Player* player) {
    ...
    // Swap Characters
    if (player->form == FORM_ON_FOOT) {
        if ((gControllerPress[player->num].button & L_JPAD) && !(gControllerHold[player->num].button & R_JPAD) && !(gControllerHold[player->num].button & U_JPAD) && !(gControllerHold[player->num].button & D_JPAD)) {
            gPilotNum--;
        }
        if ((gControllerPress[player->num].button & R_JPAD) && !(gControllerHold[player->num].button & L_JPAD) && !(gControllerHold[player->num].button & U_JPAD) && !(gControllerHold[player->num].button & D_JPAD)) {
            gPilotNum++;
        }
        if (gPilotNum > 3) {
            gPilotNum = 0;
    ...
    switch (player->state) {
        ...
        case PLAYERSTATE_LEVEL_INTRO:
            ...
            if (gTurretModeEnabled) {
                ...
                if (player->grounded == false) {
                ...
                } else if ((gCurrentLevel == LEVEL_MACBETH) || (gCurrentLevel == LEVEL_VENOM_1) || (gCurrentLevel == LEVEL_VENOM_2)) {        // Running
                    Vec3f sp78[30];
                    f32 sp64;
                    s32 sp44;
                    ...
                    switch (gPilotNum) {                            // Standing Still
                        case 0:
        ...
        case PLAYERSTATE_LEVEL_COMPLETE:
            ...
            if (gTurretModeEnabled) {
                ...
                if (((gCurrentLevel != LEVEL_TITANIA) && (gCurrentLevel != LEVEL_MACBETH)) || (player->pos.y > 10.0f)) {
                    switch (gPilotNum) {                                                      // Jumping animation
                        case 0:
                            sp48 = LOAD_ASSET(D_versus_302E95C);
                            break;
                    ...
                    if ((gCurrentLevel == LEVEL_VENOM_2) && (player->state == PLAYERSTATE_LEVEL_COMPLETE) && (player->xRot_120 > 60.0f)) {
                        Audio_KillSfxById(NA_SE_TANK_GO_UP);
                ...
                } else if ((gCurrentLevel == LEVEL_MACBETH) || (gCurrentLevel == LEVEL_TITANIA)) {        // Running
                    Vec3f sp78[30];
                    f32 sp64;
                    s32 sp44;
        ...
        case PLAYERSTATE_START_360:
            ...
            if (player->form == FORM_ON_FOOT) {
                ...
                } else if ((gCurrentLevel == LEVEL_SECTOR_Y) || (gCurrentLevel == LEVEL_VENOM_ANDROSS) || (player->grounded == false)) {
                    switch (gPilotNum) {
                        case 0:
                            sp48 = LOAD_ASSET(D_versus_302E95C);
                            break;
        ...
        case PLAYERSTATE_NEXT:
            ...
            if (((gCurrentLevel == LEVEL_ZONESS) || (gCurrentLevel == LEVEL_SOLAR)) && (player->form != FORM_ON_FOOT)) {
...
void Camera_UpdateOnFoot360(Player* player, s32 arg1) {
    ...
    /* player->cam.at.x = player->pos.x + player->damageShake * 0.1f;                       // move independent from camera
    player->cam.at.z = player->pos.z + player->damageShake * 0.1f;
    ...
    if ((player->grounded == true)) { // zoom in when looking up
        Math_SmoothStepToF(&player->camDist, -(gInputPress->stick_y), 0.1f, 100.0f, 0.001f);
    }
    ...
    player->cam.at.y = player->pos.y + 18.0f + player->unk_02C * 1.5f + player->damageShake * 0.1f;
    player->cam.at.z = player->pos.z;
...
void Camera_UpdateOnFoot(Player* player, s32 arg1) {
    Vec3f sp64;
    Vec3f sp58;
    Vec3f sp4C;

    player->cam.eye.x = (player->pos.x/*  * 0.95f */);  // lag camera when you strafe left and right
    player->cam.at.x = player->pos.x + player->damageShake * 0.1f;
            ...
            switch (gLevelMode) {
                case LEVELMODE_ON_RAILS:
                    if ((player->form == FORM_ARWING) || gTurretModeEnabled) {
                        ...
                        Camera_UpdateOnFoot(player, false);
                    }
                    break;
...
void Play_UpdateLevel(void) {
    ...
    switch (gCurrentLevel) {
        ...
        case LEVEL_SOLAR:
            ...
            if ((gPlayer[0].pos.y > 600.0f) || (gPlayer[0].form == FORM_ON_FOOT)) {
                cycleMask = 8 - 1;
                heightParam = 5;
