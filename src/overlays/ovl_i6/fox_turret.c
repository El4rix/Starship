...
Gfx mat_Guns_f3dlite_material_058[] = {
	...
	gsDPSetTextureImage(G_IM_FMT_RGBA, G_IM_SIZ_16b_LOAD_BLOCK, 1, D_GREAT_FOX_E00B4B0),
...
void Turret_Shoot(Player* player) {
    ...
    for (j = 1; j < player->turretLockOnCount; j++) {
        if ((gBosses[j].obj.status == OBJ_ACTIVE) /* && (gActors[i].obj.id == OBJ_ACTOR_EVENT) */) {
            ...
            gTexturedLines[j + 4].posAA.y = player->pos.y;
...
void Turret_UpdateRails(Player* player) {
    ...
    switch (player->form) {
        case FORM_LANDMASTER:
            ...
            // Has to do with broken texture scrolling on Titania, Macbeth? Fixed elsewhere.
            /* if (player->baseSpeed > 0.0f) {
    ...
    if ((player->shields <= 0) && (player->radioDamageTimer != 0)) {
        ...
        if (player->csEventTimer == 120) {
            Effect_Effect383_Spawn(player->pos.x, player->pos.y, player->pos.z - 2000, 80.0f);
            for (i = 0; i < 100; i++) {
                func_effect_8007D0E0(player->pos.x, player->pos.y, player->pos.z - 400, 10.0f);
                func_effect_80079618(RAND_FLOAT_CENTERED(300.0f) + player->pos.x,
                player->pos.y - RAND_FLOAT(2000.0f),
                                        RAND_FLOAT_CENTERED(300.0f) + player->pos.z - 1000, 5.11f);
            };

            for (i = 0; i < 100; i++) {
                Effect_Effect357_Spawn95(RAND_FLOAT_CENTERED(300.0f) + player->pos.x,
                                        player->pos.y - RAND_FLOAT(2000.0f),
                                            RAND_FLOAT_CENTERED(300.0f) + player->pos.z - 2000, 5.11f);
            };
        }
    }
    ...
    // Correct for Tank collision sending you too far backwards
...
void Turret_Update360(Player* player) {
    ...
    if ((player->shields <= 0) || (player->damage >= player->shields)/*  && (player->radioDamageTimer != 0) */) {
        ...
        player->csEventTimer = 120;
        if (player->csEventTimer == 120) {
            Effect_Effect383_Spawn(player->cam.at.x + (player->cam.at.x - player->cam.eye.x), player->pos.y, player->cam.at.z + (player->cam.at.z - player->cam.eye.z), 80.0f);
            func_effect_8007D0E0(player->pos.x, player->pos.y, player->pos.z, 10.0f);
            for (i = 0; i < 100; i++) {
                func_effect_80079618(RAND_FLOAT_CENTERED(300.0f) + player->pos.x,
                player->pos.y - RAND_FLOAT(2000.0f),
                                        RAND_FLOAT_CENTERED(300.0f) + player->pos.z, 5.11f);
            };
